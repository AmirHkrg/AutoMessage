...
def sendInstaDirect(message: str, time_hour: int, time_min: int, repeat_num: int, wait_time: int = 10,
                         tab_close: bool = False, close_time: int = 3) -> None:
    ...
    if left_time < wait_time:
        raise CallTimeException(
            "Call time must be greater than wait_time as instagram.com takes some time to load")

    date = "%s:%s:%s" % (current_time.tm_mday, current_time.tm_mon, current_time.tm_year)
    time_write = "%s:%s" % (timehr, time_min)
    with open("InstagramDirect_history.txt", "a", encoding='utf-8') as file:
        file.write("Date: %s\nTime: %s\nMessage: %s\nRepeat Number" %
                   (date, time_write, message, repeat_num))
        file.write("\n--------------------\n")
    sleep_time = left_time - wait_time
    print(f"\nIn {sleep_time} seconds instagram.com will open and after {wait_time} seconds message will be delivered")
    time.sleep(sleep_time)
    web.open('https://www.instagram.com/direct/inbox/')
    time.sleep(10)
    width, height = pg.size()
    pg.click(width / 4, height - height / 1.5)
    pc.copy(message)
    time.sleep(10)
    width, height = pg.size()
    pg.click(width / 2, height - height / 8)
    time.sleep(2)
...
def run():
    ...
    if(command_input == "1"):
        ...
        if(Time_Min == ""):
            ...
            print(Fore.WHITE)
            exit()
        repeat_num = int(input("Enter the number of messages sent : "))
        if(repeat_num == ""):
            print("Field is mandatory")
            print(Fore.WHITE)
            exit()
        sendInstaDirect(Message_Text, Time_hr, Time_Min, repeat_num)
...
print("<<  To send an automatic message to a Instagram Direct ......... select 4  >>")
